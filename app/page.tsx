...
import { getPageQuery, getHomePageQuery } from "@/sanity/lib/queries";

export default async function Home() {
  const pageContent = await client.fetch(getPageQuery, { slug: '/' });
  const homePageData = await client.fetch(getHomePageQuery);
    ...
    <div className="w-full">
      ...
      <section className="w-full py-16 md:py-24 px-6 lg:px-8 bg-background/60 backdrop-blur-sm relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto space-y-20 md:space-y-32 relative z-10">
          {(homePageData?.mainFeatures || [
            { title: "Advanced Large Format 3D Printing", description: "From large sculptures to scale models, we use our state-of-the-art large format 3D printing technology to push the size limitation offered by traditional 3D printing companies. Crafting forms by combining design, technology and innovation.", imageUrl: "/assets/3d1.png" },
            { title: "Merging Sustainability And Affordability", description: "Our solutions are sustainable, and affordable. We are a part of the circular economy that brings your creative ambitions to life.", imageUrl: "/assets/3d2.png" },
            { title: "Uae Based Manufacturing, Serving Globally.", description: "We are a home grown company based in Dubai; Rely on our local expertise for your global challenges. We ensure that every solution provided reflects our commitment to quality.", imageUrl: "/assets/3d3.png" },
          ]).map((feature: any, i: number) => (
            <div key={i} className="grid lg:grid-cols-2 gap-10 md:gap-16 items-center">
              <FadeIn direction={i % 2 === 0 ? "right" : "left"} delay={i % 2 === 0 ? 0 : 0.2} className={i % 2 === 0 ? "" : "order-1 lg:order-2"}>
                <h2 className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-foreground mb-4 md:mb-6">{feature.title}</h2>
                <p className="text-base md:text-lg text-foreground/70 font-josefin font-light leading-relaxed">
                  {feature.description}
                </p>
              </FadeIn>
              <FadeIn direction={i % 2 === 0 ? "left" : "right"} delay={i % 2 === 0 ? 0.2 : 0} className={`h-full w-full ${i % 2 === 0 ? "" : "order-2 lg:order-1"}`}>
                <div className="w-full aspect-square bg-border rounded-3xl overflow-hidden relative shadow-2xl">
                  <Image src={feature.imageUrl} alt={feature.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                </div>
              </FadeIn>
            </div>
          ))}
      ...
      <section className="w-full py-16 md:py-24 px-6 lg:px-8 bg-primary text-primary-foreground relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 relative z-10 text-center">
          {(homePageData?.stats || [
            ...
            { number: "20k+", label: "Pellets Used" },
            { number: "6k+", label: "Industry Experience" },
          ]).map((stat: any, i: number) => (
            <FadeIn key={i} delay={0.1 * (i + 1)}>
              <div className="text-4xl md:text-5xl lg:text-6xl font-heading font-bold mb-2 md:mb-4">{stat.number}</div>
              <div className="text-primary-foreground/80 font-josefin tracking-wider uppercase text-xs md:text-sm">{stat.label}</div>
            </FadeIn>
          ))}
        </div>
      </section>
      ...
      <section className="w-full py-20 md:py-32 px-6 lg:px-8 bg-transparent">
          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {(homePageData?.whyChooseUs || [
              { title: "Advanced Pellet Printers", description: "Our state-of-the-art pellet printers offer unmatched precision and quality, enabling the creation of intricate and robust large format 3D prints." },
              { title: "Large Format Capabilities", description: "Our customizable large format 3D printers can handle projects up to 3 meters in height, perfect for ambitious and sizable designs." },
              { title: "Top-of-the-Line Printers", description: "For smaller elements, we use high-precision FDM 3D printers to ensure meticulous detail and superior quality." },
              { title: "Expert Team", description: "Our talented designers and fabricators bring expertise and creativity to every project, ensuring exceptional results from start to finish." },
            ]).map((feature: any, i: number) => (
              <FadeIn key={feature.title} delay={0.1 * i} direction="up" className="bg-transparent p-6 md:p-8 rounded-3xl border border-border/50 hover:border-primary/50 transition-colors">
                ...
                <p className="text-foreground/70 font-josefin font-light leading-relaxed">{feature.description || feature.desc}</p>
              </FadeIn>
            ))}
          </div>
        </section>
      ...
      <section className="w-full py-20 md:py-32 px-6 lg:px-8 bg-muted">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {(homePageData?.ourServices || [
                { title: "Advanced 3D Print Solutions", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798bc3f5f28106b27f69b66_75252de1259e2a79bd20bc62fca24c51_POT.png" },
                { title: "Bespoke Design Solution", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798bc1ba1a6bbd3ca34f680_WhatsApp%20Image%202024-09-24%20at%2015.29.04_1a69ca40.jpg" },
                { title: "Scale Model Design", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798bb6bca15c9ef87f7bcc0_WhatsApp%20Image%202024-07-16%20at%201.31.05%20PM-4.jpeg" },
                { title: "Architectural Elements", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798bc627e12990f3b50f80b_ac9efa2625400959a43aa6a8483660dd_ID.png" },
                { title: "3D Printed Furniture", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798bc764063727d590a1b1c_IMG_1062.JPG" },
                { title: "Retail Displays", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798bbc1818843b7e0a75d6c_820f1eac47e65ca457ad567c07691576_IMG-20241105-WA0039.avif" },
                ...
                <FadeIn key={service.title} delay={0.1 * i} direction="up" className="group cursor-pointer">
                  <Link href="/services" className="block w-full h-full">
                    <div className="relative w-full aspect-square rounded-3xl overflow-hidden mb-4 md:mb-6">
                      ...
                      <Image src={service.imageUrl || service.img} alt={service.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover group-hover:scale-110 transition-transform duration-700" />
        ...
        >
          <div className="flex items-center h-[50vh] md:h-[60vh] max-h-[600px]">
            {(homePageData?.ourIndustries || [
              { title: "Art & Figurines", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/679c83dc9d826171b90153b8_greger.png" },
              { title: "Sculptures", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798d60700323216feaa583d_0284f828223d6f3f5000536e9914d54b_PEACOCK%20FRONT.png" },
              { title: "Scale Models", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/6798bb6bca15c9ef87f7bcc0_WhatsApp%20Image%202024-07-16%20at%201.31.05%20PM-4.jpeg" },
              { title: "Retail Display", imageUrl: "https://cdn.prod.website-files.com/66da0818e9f24d66d344680f/679b72388458dc748e83e1bc_b2aa567d67b672de17f0dd53ae3b0416_GISOU%20NY%20SIDE.png" },
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-10 md:gap-8">
              {(homePageData?.processSteps || [
                { step: "01", title: "Talk to us", description: "Let us know what you have in your mind, and we can suggest the best solution." },
                { step: "02", title: "Share Design", description: "Select from our vast library or share your custom design." },
                { step: "03", title: "3D Printing", description: "We ensure each print reflects superior quality and precision." },
                { step: "04", title: "Perfect Details", description: "Our post processing team ensures every detail is perfected." },
                { step: "05", title: "Delivered", description: "Your vision is now a reality and will be shipped to you shortly." },
                ...
                <FadeIn key={item.step} delay={0.1 * i} direction="up" className="flex flex-col items-center text-center">
                  <div className="w-16 h-16 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-heading font-bold text-xl mb-4 md:mb-6 relative">
                    {item.step}
                    {i < (homePageData?.processSteps?.length || 5) - 1 && <div className="hidden md:block absolute top-1/2 left-full w-full h-0.5 bg-primary/30 -z-10 -translate-y-1/2" />}
                  </div>
                  <h3 className="text-lg font-heading font-bold text-foreground mb-2">{item.title}</h3>
                  <p className="text-sm text-foreground/70 font-josefin">{item.description || item.desc}</p>
                </FadeIn>
              ))}
            </div>
          </div>
